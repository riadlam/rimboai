...
export default function ImageLabPreviewModal({
    ...
    const handleUseLastFrame = async () => {
        ...
        try {
            // Capture only on click — never proxy the preview player (keeps playback fast).
            const frameFile = await captureVideoLastFrameFile(mediaUrl, {
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-0 sm:p-4 [&_a]:cursor-pointer [&_button]:cursor-pointer"
            onClick={onClose}
        >
            <motion.div

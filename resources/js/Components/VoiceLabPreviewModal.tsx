...
export default function VoiceLabPreviewModal({
    voice,
    index,
    total,
    onClose,
    ...
    const handleDownload = async () => {
        if (!voice.audioUrl || downloading) return;
        setDownloading(true);
        try {
            await downloadAsset(voice.audioUrl, `voice-${voice.id}.mp3`);
        } finally {
    ...
    const palette = musicPalette(voice.id);
            ...
            >
                <div className="relative flex h-[52vh] shrink-0 flex-col overflow-hidden bg-black md:h-full md:flex-1">
                    ...
                    <div className="relative z-10 shrink-0 border-t border-white/10 bg-gradient-to-t from-black via-[#0a0a0f] to-[#0a0a0f]/90 px-3 py-3 md:px-5 md:py-4">
                        {voice.audioUrl ? (
                            ...
                            <div className="rounded-xl border border-dashed border-white/10 px-4 py-5 text-center text-[13px] text-white/40">
                                No audio file available for this voice yet.
                            </div>
                        )}
                    </div>
                    ...
                    <div className="absolute bottom-[5.5rem] start-3 z-10 flex max-w-[calc(100%-1.5rem)] flex-row flex-wrap items-center gap-2 md:bottom-[6.25rem] md:start-5">
                        <div className="flex items-center gap-0.5 rounded-xl border border-white/10 bg-[#121217]/85 p-1 shadow-lg backdrop-blur-md">
                            ...
                            <PreviewIconBtn
                                title={voice.favorite ? t('unfavorite') : t('favorite')}
                                onClick={() => onToggleFavorite?.(voice.id)}
                            >
                                <svg
                                    className={`h-4 w-4 ${voice.favorite ? 'fill-[#FF5733] text-[#FF5733]' : ''}`}
                    ...
                    {total > 1 && onPrev && onNext && (
                        <>
                            <NavArrow direction="prev" label={t('library.previous')} onClick={onPrev} />
                            <NavArrow direction="next" label={t('library.next')} onClick={onNext} />
                        </>
                    )}
                </div>
                ...
                <div className="flex min-h-0 flex-1 flex-col overflow-hidden border-t border-white/10 bg-[#111116] font-sans md:w-80 md:flex-none md:border-s md:border-t-0">
                    ...
                    <div className="flex shrink-0 items-center justify-between border-b border-white/10 px-4 py-3 md:hidden">
                        <span className="truncate text-[13px] font-semibold text-zinc-100">{voice.title}</span>
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-thin">
                        <div className="space-y-2.5 p-4">
                            <div>
                                ...
                                <h3 className="mt-1 text-base font-semibold tracking-tight text-white">{voice.title}</h3>
                            </div>

                            <div className="grid grid-cols-2 gap-2">
                                <DetailBtn onClick={() => void handleDownload()} disabled={!voice.audioUrl || downloading}>
                                    <IconDownload className="h-3.5 w-3.5 opacity-70" />
                                    {downloading ? t('library.downloading') : t('download')}
                                </DetailBtn>
                                <DetailBtn gradient onClick={() => onToggleFavorite?.(voice.id)}>
                                    <svg className="h-3.5 w-3.5" fill={voice.favorite ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                                        <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                                    </svg>
                                    {voice.favorite ? t('library.favorited') : t('favorite')}
                                </DetailBtn>
                            </div>
                        </div>
                            ...
                            {detailsOpen && (
                                <div className="space-y-3 px-4 pb-4">
                                    <div className="flex items-center justify-between gap-3 text-[13px]">
                                        ...
                                        <span className="truncate text-end font-medium text-[#FF5733]" title={voice.model || '—'}>
                                            {voice.model || '—'}
                                        </span>
                                    </div>
                                    <div className="flex items-center justify-between gap-3 text-[13px]">
                                        <span className="shrink-0 text-zinc-500">Voice</span>
                                        <span className="truncate text-end font-medium text-zinc-200" title={voice.voice}>
                                            {voice.voice}
                                        </span>
                                    </div>
                                    {voice.duration && (
                                        <div className="flex items-center justify-between gap-3 text-[13px]">
                                            ...
                                            <span className="truncate text-end font-medium text-zinc-200">{voice.duration}</span>
                                        </div>
                                    )}
                                    <div className="space-y-1.5">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[13px] text-zinc-500">Script</span>
                                            <button
                                                type="button"
                                                title={copied ? t('library.copied') : t('library.copyText')}
                                        ...
                                        <div className="rounded-lg bg-white/[0.03] p-3 ring-1 ring-inset ring-white/5">
                                            <p className="line-clamp-8 text-[13px] leading-relaxed text-zinc-300">{voice.text || '—'}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between text-xs text-zinc-600">
                                        <span>{t('library.detail.textToSpeech')}</span>
                                        <span>
                                            {index + 1} / {total}
                                        </span>
                                    </div>
